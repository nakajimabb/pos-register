...
type Props = {
  open: boolean;
  value: InventoryDetail | undefined;
  onClose: () => void;
  onUpdate: (deliveryDetail: InventoryDetail) => void;
};

const InventoryDetailEdit: React.FC<Props> = ({ open, value, onClose, onUpdate }) => {
  const [deliveryDetail, setInventoryDetail] = useState<InventoryDetail>(
    value || {
      productCode: '',
      productName: '',
      quantity: 0,
        ...
        <Modal.Body>
          <Grid cols="1 sm:2" gap="0 sm:3" auto_cols="fr" template_cols="1fr 2fr" className="row-end-2">
            ...
            <Form.Number
              ...
              onChange={(e) => {
                setInventoryDetail({ ...deliveryDetail, quantity: +e.target.value });
              }}
            />
          </Grid>
        </Modal.Body>
